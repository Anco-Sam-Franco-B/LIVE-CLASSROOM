...
function CertificatePreview({ template, signatureUrl, authorityName, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div className="max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" style={{ background: 'var(--bg-card)', borderRadius: '16px' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: 'var(--border-neon)' }}>
          <h2 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Certificate Preview: {template.name}</h2>
          <button onClick={onClose} className="p-2 rounded-lg" style={{ color: 'var(--text-muted)' }} onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,255,65,0.05)';e.currentTarget.style.color='var(--text-primary)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}}><X size={20} /></button>
        </div>
        <div className="p-8 flex justify-center" style={{ background: 'var(--bg-dark)' }}>
          <div
          ...
          >
            ...
            <div className="relative h-full flex flex-col items-center justify-center px-16 py-10">
              ...
              <p className="text-sm mb-2" style={{ color: 'var(--text-muted)' }}>This is to certify that</p>
              {template.show_student_name && (
                <h2 className="text-2xl font-bold mb-3" style={{ color: template.font_color }}>
                  {previewData.studentName}
                </h2>
              )}
              <p className="text-sm mb-2" style={{ color: 'var(--text-muted)' }}>has successfully completed the course</p>
              ...
              {template.custom_text && (
                <p className="text-xs italic mb-6 max-w-lg text-center leading-relaxed" style={{ color: 'var(--text-muted)' }}>{template.custom_text}</p>
              )}
                ...
                <div className="flex flex-col">
                  ...
                  {signatureUrl ? (
                    <div className="flex flex-col items-start gap-1">
                      ...
                      <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{authorityName || 'Authorized Signature'}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <div className="w-24 h-0.5" style={{ backgroundColor: template.accent_color }} />
                      <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Authorized Signature</span>
                    </div>
                  )}
                </div>
                <div className="flex flex-col items-center gap-1">
                  <div className="p-1.5 rounded-lg shadow-sm" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-neon)' }}>
                    <QRCodeSVG value={qrValue} size={56} level="M" fgColor={template.font_color} />
                  </div>
                  <span className="text-[9px] tracking-wider" style={{ color: 'var(--text-muted)' }}>SCAN TO VERIFY</span>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-center gap-4 p-4 border-t text-xs" style={{ borderColor: 'var(--border-neon)', color: 'var(--text-muted)' }}>
...
export default function CertificateTemplates() {
  ...
  const [form, setForm] = useState({ name: '', description: '', fontFamily: 'Arial', fontColor: '#1a1a2e', accentColor: '#00ff41', showStudentName: true, showCourseName: true, showDate: true, showGrade: true, showDuration: true, customText: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ name: '', description: '', fontFamily: 'Arial', fontColor: '#1a1a2e', accentColor: '#00ff41', showStudentName: true, showCourseName: true, showDate: true, showGrade: true, showDuration: true, customText: '' });
      load();
  ...
  return (
    <div>
      ...
      <PageHeader title="Certificate Templates" description="Design and manage certificate layouts" actions={!showForm && <button onClick={() => { setEditing(null); setForm({ name: '', description: '', fontFamily: 'Arial', fontColor: '#1a1a2e', accentColor: '#00ff41', showStudentName: true, showCourseName: true, showDate: true, showGrade: true, showDuration: true, customText: '' }); setShowForm(true); }} className="neon-btn flex items-center space-x-2"><Plus size={20} /><span>New Template</span></button>} />

      {showForm && (
        <div className="neon-card mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>{editing ? 'Edit Template' : 'Create Template'}</h2>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Show on Certificate</label>
            ...
            <div className="flex space-x-3">
              <button type="submit" className="neon-btn">{editing ? 'Update' : 'Create'} Template</button>
      ...
      {templates.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map((t) => (
            <div key={t.id} className={`neon-card relative ${!t.is_active ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: 'rgba(0,255,65,0.1)' }}>
                  <FileText size={20} style={{ color: 'var(--neon)' }} />
                </div>
                <div className="flex items-center space-x-1">
                  <button onClick={() => setPreview(t)} className="p-1.5 rounded-lg" style={{ color: 'var(--text-muted)' }} onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,255,65,0.05)';e.currentTarget.style.color='var(--neon)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}} title="Preview"><Eye size={16} /></button>
                  <button onClick={() => toggleActive(t)} className="p-1.5 rounded-lg" style={{ color: 'var(--text-muted)' }} onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,255,65,0.05)';e.currentTarget.style.color='var(--text-primary)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}} title={t.is_active ? 'Deactivate' : 'Activate'}>{t.is_active ? <EyeOff size={16} /> : <Eye size={16} />}</button>
                  <button onClick={() => handleEdit(t)} className="p-1.5 rounded-lg" style={{ color: 'var(--text-muted)' }} onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,255,65,0.05)';e.currentTarget.style.color='var(--neon)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}} title="Edit"><Edit2 size={16} /></button>
                  <button onClick={() => setConfirmDelete(t.id)} className="p-1.5 rounded-lg" style={{ color: 'var(--text-muted)' }} onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,255,65,0.05)';e.currentTarget.style.color='#ff3232'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}} title="Delete"><Trash2 size={16} /></button>
                </div>
              </div>
              <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>{t.name}</h3>
              {t.description && <p className="text-sm mt-1 line-clamp-2" style={{ color: 'var(--text-secondary)' }}>{t.description}</p>}
              <div className="flex items-center space-x-3 mt-3 text-xs" style={{ color: 'var(--text-muted)' }}>
                <span>Font: {t.font_family}</span>
                <span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: t.accent_color }} />
                <span className={`px-2 py-0.5 rounded-full ${t.is_active ? 'neon-badge-success' : 'neon-badge-info'}`}>{t.is_active ? 'Active' : 'Inactive'}</span>
              </div>
            </div>
          ))}
        </div>
