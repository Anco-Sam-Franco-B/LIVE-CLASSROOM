...
export default function TeacherAnnouncements() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([announcementsAPI.getAll(), coursesAPI.getTeacherCourses()])
  ...
  const priorityBadge = (p) => {
    const map = { urgent: 'neon-badge-danger', high: 'neon-badge-warning', normal: 'neon-badge-info', low: 'neon-badge-ghost' };
        ...
        <div className="neon-card mb-8" style={{ background: 'var(--bg-card)' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <Select label="Target" value={form.targetAudience} onChange={e => setForm({ ...form, targetAudience: e.target.value })}>
                <option value="all">All</option><option value="students">Students Only</option><option value="course">Specific Course</option>
              </Select>
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-1 ml-4">
                  <button onClick={() => handleEdit(a)} className="p-1.5 rounded-lg" style={{ color: 'var(--text-muted)' }}
                    ...
                    onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}}><Edit2 size={16} /></button>
