...
const { sendEnrollmentEmail, sendCertificateEmail } = require("../services/emailService");
...
const getCourseStudents = async (req, res, next) => {
  try {
    const result = await pool.query(
      `SELECT e.id as enrollment_id, u.id as user_id, u.first_name, u.last_name, u.email, u.avatar_url, e.enrolled_at, e.progress, e.completed_lessons, e.total_lessons, e.is_completed, e.status
...
const updateLessonProgress = async (req, res, next) => {
  try {
    ...
    const enrollment = await pool.query(
      "SELECT id, user_id, course_id FROM enrollments WHERE user_id = $1 AND course_id = (SELECT cm.course_id FROM lessons l JOIN course_modules cm ON l.module_id = cm.id WHERE l.id = $2)",
      [req.userId, lessonId]
    );
    ...
    // Auto-issue certificate if course is now completed
    const enrollResult = await pool.query(
      `SELECT e.*, c.title as course_title, c.slug as course_slug, c.duration_hours,
              u.first_name, u.last_name, u.email
       FROM enrollments e
       JOIN courses c ON e.course_id = c.id
       JOIN users u ON e.user_id = u.id
       WHERE e.id = $1 AND e.is_completed = true`,
      [enrollment.rows[0].id]
    );

    if (enrollResult.rows.length > 0) {
      const enr = enrollResult.rows[0];
      const existing = await pool.query("SELECT id FROM certificates WHERE enrollment_id = $1", [enr.id]);
      if (existing.rows.length === 0) {
        const certResult = await pool.query(
          "INSERT INTO certificates (user_id, course_id, enrollment_id, grade, certificate_url) VALUES ($1, $2, $3, $4, $5) RETURNING *",
          [enr.user_id, enr.course_id, enr.id, "A", "/certificates/" + enr.course_slug + "-" + Date.now() + ".pdf"]
        );
        ...
        const qrData = JSON.stringify({
          ...
          student: enr.first_name + " " + enr.last_name,
          course: enr.course_title,
        ...
        } catch (qrErr) { console.error("QR generation failed:", qrErr.message); }
        sendCertificateEmail({ first_name: enr.first_name, last_name: enr.last_name, email: enr.email }, cert, enr).catch(() => {});
      }
    }
...
const uploadThumbnail = async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      "UPDATE courses SET thumbnail_url = $1, updated_at = NOW() WHERE id = $2 RETURNING *",
